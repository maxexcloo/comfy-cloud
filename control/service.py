...
from control.capacity_manager import CapacityManager
...
from control.preferences import ConfigurationManager, ControlPreferences
from control.store import ControlStore, Job
from control.video_queue import VideoQueue
from providers.base import StartRecovery
from providers.cliproxy import CliproxyClient
from providers.deployment import deployment_options
from providers.deployment.common import (
    DeploymentSelection,
    worker_model_profiles,
)
from providers.modal import (
...
class ProviderRuntime:
    ...
    capacity_checked_at: float = 0
    capacity_cost_per_hour: float | None = None
...
class Controller:
    def __init__(self, settings: ControlSettings):
        ...
        self.capacity = CapacityManager(self)
        self.videos = VideoQueue(self)
    ...
    async def run_video(self, job: Job) -> None:
        ...
        try:
            ...
            if not job.provider and len(targets) > 1:
                targets = await self.economical_targets(targets)
            for target in targets:
                ...
                try:
                    ...
                    if target.provider == "cliproxyapi":
                        self.store.start_phase(job.id, "running")
                ...
                if (response is None or response.is_success) and data.get(
                    ...
                    self.store.start_phase(job.id, "archiving")
                    ...
                    self.store.finish_phases(job.id)
                    return
    ...
    async def _run_provider_action(
        ...
        async with runtime.lock:
            ...
            if action_name == "deploy":
                ...
                selected_preferences = preferences or self.preferences
                for profile in worker_model_profiles(
                    runtime.config, selected_preferences
                ):
                    if profile == "image-upscale":
                        continue
    ...
    async def economical_targets(self, targets: list[Target]) -> list[Target]:
        return await self.capacity.economical_targets(targets)
