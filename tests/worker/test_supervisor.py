...
from worker.logs import capture_process_logs, entries
from worker.supervisor import _comfy_arguments, _prepare_models, run
...
def test_configured_model_profile_is_prepared(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "worker.supervisor.fetch_profile",
...
def test_empty_model_selection_prunes_all_managed_profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "worker.supervisor.prune_profiles",
...
def test_gateway_starts_before_model_preparation(monkeypatch):
    ...
    monkeypatch.setattr(
        "worker.supervisor._start_gateway",
        lambda *_: events.append("gateway") or Gateway(),
    )
    monkeypatch.setattr(
        "worker.supervisor._prepare_models",
