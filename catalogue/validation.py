...
from catalogue.workflows import Catalogue, workflow_operation_names
...
def validate_repository(catalogue_dir: Path) -> None:
    ...
    for path in sorted((catalogue_dir / "profiles").glob("*.yaml")):
        ...
        ProfilePolicy.model_validate(profile)
        ...
        recommended_vram = profile.get("recommended_vram_gb")
        if not isinstance(recommended_vram, int) or recommended_vram < minimum_vram:
            raise ValueError(
                f"profile recommended_vram_gb must be at least its minimum: {name}"
            )
        if profile.get("service_class") not in {"batch", "interactive"}:
            raise ValueError(f"profile must declare a service class: {name}")
        if path.stem != name:
