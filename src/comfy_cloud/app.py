...
NATIVE_API_PREFIXES = (
    "prompt", "history", "view", "queue", "object_info", "system_stats", "upload", "interrupt", "free", "userdata", "api",
)


def openai_error(message: str, status: int, code: str, param: str | None = None) -> JSONResponse:
    return JSONResponse(
        status_code=status,
        content={"error": {"message": message, "type": "invalid_request_error", "param": param, "code": code}},
    )
...
class Runtime:
    ...
    async def run(self, model: WorkflowModel, values: dict[str, Any]) -> list[OutputRef]:
        graph = model.render(values)
        async with self.inference_lock:
            prompt_id = await self.comfy.submit(graph)
            return await self.comfy.wait(prompt_id, model.output.node, self.settings.workflow_timeout)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def model_object(model: WorkflowModel) -> dict[str, Any]:
        return {"id": model.id, "object": "model", "created": 0, "owned_by": model.owned_by}

    @app.get("/v1/models")
    async def models(request: Request) -> Response:
        if denied := require(request):
            return denied
        return JSONResponse({"object": "list", "data": [model_object(model) for model in runtime.catalog.list_available(settings.models_dir)]})
    ...
    async def retrieve_model(model_id: str, request: Request) -> Response:
        ...
        try:
            ...
            return openai_error(f"The model '{model_id}' does not exist", 404, "model_not_found", "model")
        value = model_object(model)
        value["capabilities"] = {
            "operation": model.operation,
            "input_modalities": ["text", "image"] if "image" in model.input_map else ["text"],
    ...
    async def image_generations(request: Request) -> Response:
        ...
        try:
            ...
            return openai_error("Requested model was not found", 404, "model_not_found", "model")
        except ValueError:
            return openai_error("Request body must be JSON", 400, "invalid_json")
        if model.operation != "image_generation":
            return openai_error(f"Model '{model.id}' does not support image generation", 400, "unsupported_operation", "model")
        prompt = body.get("prompt")
        if not isinstance(prompt, str) or not prompt:
            return openai_error("prompt is required", 400, "missing_required_parameter", "prompt")
        size = body.get("size")
        width = height = None
        if size and size != "auto":
            try:
                ...
                return openai_error("size must be WIDTHxHEIGHT or auto", 400, "invalid_value", "size")
        values = {
            "prompt": prompt,
    ...
    async def image_edits(request: Request) -> Response:
        ...
        if model.operation != "image_edit":
            return openai_error(f"Model '{model.id}' does not support image editing", 400, "unsupported_operation", "model")
        image = form.get("image")
        if image is None or not hasattr(image, "read"):
            return openai_error("image is required", 400, "missing_required_parameter", "image")
        uploaded = await runtime.comfy.upload(image.filename, await image.read(), image.content_type or "application/octet-stream")
        refs = await runtime.run(model, {"prompt": str(form.get("prompt", "")), "image": uploaded})
        output = await runtime.comfy.fetch_output(refs[0])
        return JSONResponse({"created": int(time.time()), "data": [{"b64_json": base64.b64encode(output.content).decode()}]})

    async def execute_video(job: VideoJob, model: WorkflowModel, values: dict[str, Any]) -> None:
        try:
    ...
    async def create_video(request: Request) -> Response:
        ...
        if model.operation != "video_generation":
            return openai_error(f"Model '{model.id}' does not support video generation", 400, "unsupported_operation", "model")
        values = dict(body)
        size = body.get("size")
        if size and size != "auto":
            try:
                values["width"], values["height"] = (int(part) for part in size.lower().split("x", 1))
        ...
        if body.get("seconds") is not None:
            try:
                ...
                return openai_error("seconds must be a number", 400, "invalid_value", "seconds")
            if seconds <= 0 or seconds > 15:
                return openai_error("seconds must be between 0 and 15", 400, "invalid_value", "seconds")
        ...
        return JSONResponse({"id": job.id, "object": "video", "model": job.model, "status": job.status, "created_at": job.created_at})
    ...
    async def get_video(job_id: str, request: Request) -> Response:
        ...
        return JSONResponse({"id": job.id, "object": "video", "model": job.model, "status": job.status, "created_at": job.created_at, "error": job.error})
    ...
    async def video_content(job_id: str, request: Request) -> Response:
        ...
        return Response(output.content, media_type=output.headers.get("content-type", job.output.media_type))
    ...
    async def websocket_proxy(websocket: WebSocket) -> None:
        ...
        target = settings.comfy_url.replace("http://", "ws://").replace("https://", "wss://") + "/ws"
    ...
    async def comfy_proxy(path: str, request: Request) -> Response:
        is_native = any(path == prefix or path.startswith(prefix + "/") for prefix in NATIVE_API_PREFIXES)
        if not settings.ui_enabled and not is_native:
            return JSONResponse({"detail": "ComfyUI frontend is disabled in serverless mode"}, status_code=404)
        if not request_authorized(request, settings, allow_basic=settings.ui_enabled):
            return Response(status_code=401, headers={"WWW-Authenticate": 'Basic realm="ComfyUI", Bearer'})
        body = await request.body()
        headers = {key: value for key, value in request.headers.items() if key.lower() not in HOP_HEADERS | {"host", "authorization", "content-length"}}
        ...
        response_headers = {key: value for key, value in upstream.headers.items() if key.lower() not in HOP_HEADERS | {"content-length", "content-encoding"}}
